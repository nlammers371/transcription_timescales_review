...
% for various activoator binding models discussed in the text. These
...
max_num_rate_lim_steps = 5; % number of rate-limiting steps 
n_mol = 6.02214076e23;
kb = 1.38064852e-23;
T = 300;
deltaGScale = 5000 /n_mol/kb/T;
...
for eb = 1:length(activatorEnergieArray)  
  ...
  af = off_rate_basal/(nansum(state_probs_coop.*rate_coop_off_temp(:,1)'./n_bound_vec));
...
for n = 1:max_num_rate_lim_steps
  ...
  for eb = 1:n_calc_points
    ...
    for m = 1:n   
      ...
      % add exchange terms
      Q_slice(linear_indices) = slow_kinetics_array(eb,1)*n; % on rate
